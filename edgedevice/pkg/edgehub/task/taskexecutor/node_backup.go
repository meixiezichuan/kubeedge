...
func backup(backupPath string) error {
	...
	// backup edgedevice.db: copy from origin path to backup path
	if err := copy(config.DataBase.DataSource, filepath.Join(backupPath, "edgedevice.db")); err != nil {
		return fmt.Errorf("failed to backup db: %v", err)
	}
	// backup edgedevice.yaml: copy from origin path to backup path
	if err := copy(constants.DefaultConfigDir+"edgedevice.yaml", filepath.Join(backupPath, "edgedevice.yaml")); err != nil {
		return fmt.Errorf("failed to back config: %v", err)
	}
	// backup edgedevice: copy from origin path to backup path
	if err := copy(filepath.Join(util.KubeEdgeUsrBinPath, util.KubeEdgeBinaryName), filepath.Join(backupPath, util.KubeEdgeBinaryName)); err != nil {
		return fmt.Errorf("failed to backup edgedevice: %v", err)
	}
	return nil
}
