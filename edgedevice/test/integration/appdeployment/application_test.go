...
// Run Test cases
var _ = Describe("Application deployment in edgedevice Testing", func() {
	var UID string
	Context("Test application deployment and delete deployment", func() {
		...
		It("TC_TEST_APP_DEPLOYMENT_1: Test application deployment in edgedevice", func() {
		...
		It("TC_TEST_APP_DEPLOYMENT_2: Test List application deployment in edgedevice", func() {
			//Generate the random string and assign as a UID
			UID = "deployment-app-" + edge.GetRandomString(10)
			IsAppDeployed := HandleAddAndDeletePods(http.MethodPut, ctx.Cfg.TestManager+AppHandler, UID, []v1.Container{{Name: UID, Image: ctx.Cfg.AppImageURL[0], ImagePullPolicy: v1.PullIfNotPresent}}, v1.RestartPolicyOnFailure)
			Expect(IsAppDeployed).Should(BeTrue())
		...
		It("TC_TEST_APP_DEPLOYMENT_3: Test application deployment delete from edgedevice", func() {
			//Generate the random string and assign as a UID
			UID = "deployment-app-" + edge.GetRandomString(10)
		...
		It("TC_TEST_APP_DEPLOYMENT_4: Test application deployment delete from edgedevice", func() {
			//Generate the random string and assign as a UID
			UID = "deployment-app-" + edge.GetRandomString(10)
			for i := 0; i < 2; i++ {
				UID = "deployment-app-" + edge.GetRandomString(10)
		...
		It("TC_TEST_APP_DEPLOYMENT_5: Test application deployment delete from edgedevice", func() {
			var apps []string
			//Generate the random string and assign as a UID
			UID = "deployment-app-" + edge.GetRandomString(10)
			for i := 0; i < 2; i++ {
